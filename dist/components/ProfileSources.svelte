...
export let profileOpinions = {};
export let profileOpinionsGiven = {};
export let profileInvalidations = {};
export let profileUnavailabilities = {};
export let isLoading = false;
export let onLoadProfile;
export let onLoadFile;
let filterTokenId = "";
let searchInput = "";
let viewMode = "sources";
$: {
...
$:
  timelineEvents = (() => {
    ...
    if (!activeProfileTokenId)
      return events;
    const sources = fileSources[activeProfileTokenId]?.data || [];
    const receivedOpinions = profileOpinions[activeProfileTokenId]?.data || [];
    const givenOpinions = profileOpinionsGiven[activeProfileTokenId]?.data || [];
    const invalidations = profileInvalidations[activeProfileTokenId]?.data || [];
    const unavailabilities = profileUnavailabilities[activeProfileTokenId]?.data || [];
    for (const source of sources) {
      events.push({
    ...
    for (const inv of invalidations) {
      events.push({
        ...
        // red-500
        authorTokenId: activeProfileTokenId,
        data: { sourceUrl: inv.targetBoxId }
        // targetBoxId is used as label hint
      });
    }
    ...
    for (const op of receivedOpinions) {
      events.push({
        ...
        label: op.isTrusted ? `Received a trust vote` : `Received a distrust vote`,
        color: op.isTrusted ? "#3b82f6" : "#6b7280",
        // blue-500 or gray-500
    ...
    for (const op of givenOpinions) {
      events.push({
        timestamp: op.timestamp,
        type: "PROFILE_OPINION",
        label: op.isTrusted ? `Gave trust to a profile` : `Gave distrust to a profile`,
        color: op.isTrusted ? "#8b5cf6" : "#4b5563",
        // purple-500 or gray-600
        authorTokenId: activeProfileTokenId,
